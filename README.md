...
Ramble On is a desktop-first voice-notes application built with Vite, TypeScript, and Electron. It records microphone input, transcribes it with Gemini, turns the transcript into a polished markdown note, and lets the user reshape that note into ATP dispatches, Medium-style posts, or AI-generated videos. The desktop shell also exposes a local Knowledge Base backed by the file system and synchronized into SQLite for persistence and tool access.

The repository now supports selectable AI providers for text generation. The renderer can switch between Gemini, OpenAI, and Anthropic for text workflows, uses Gemini or OpenAI for transcription, and keeps video generation on Gemini.

## What This Repository Contains

The repository is primarily a TypeScript/Electron application with a small Python smoke-test entry point and lightweight Vitest coverage.

Core runtime files:

- `src/main.tsx`: Renderer application controller. Owns the UI, recording flow, AI calls, playback, theming, and Knowledge Base interactions.
- `src/index.css`: Complete application styling for the renderer.
- `src/types.d.ts`: Renderer-side typings for the Electron preload bridge.
- `electron/main.cjs`: Electron main process. Owns window creation, SQLite setup, IPC handlers, Knowledge Base file management, and filesystem watching.
- `electron/preload.cjs`: Safe bridge that exposes the IPC-backed persistence and Knowledge Base API to the renderer.
- `electron/mcp-server.cjs`: Local MCP-over-HTTP server that exposes translation and Knowledge Base tools to external clients.
- `index.html`: Static renderer shell and the DOM contract that `VoiceNotesApp` binds to.
- `vite.config.ts`: Vite config, environment wiring, and Vitest configuration.
- `tests/*.test.ts`: Basic static and DOM smoke tests.
- `main.py`: Minimal Python entry point used for packaging or environment smoke tests.
...
- Binding all static DOM nodes from `index.html`
- Managing note state in memory and in `localStorage`
- Handling microphone capture through `MediaRecorder`
- Running transcription, polishing, ATP formatting, Medium formatting, and video generation through `@google/genai`
- Routing text generation through a provider abstraction for Gemini, OpenAI, and Anthropic
- Rendering markdown with `marked`
- Managing audio playback and speech synthesis
- Rendering and editing the local Knowledge Base
- Coordinating modal workflows and tab navigation
...
`desktop:dev` runs Vite and Electron together. Electron points at the Vite dev server using `VITE_DEV_SERVER_URL`.
...
Platform-specific macOS package:
...
- `dist/`: Vite web assets
- `dist-electron/`: Electron Builder packages
...
- `@google/genai`: Gemini transcription, text transformation, and video generation
- `marked`: Markdown rendering
- `better-sqlite3`: Local desktop persistence
- `chokidar`: Knowledge Base filesystem watch
...
Relevant variables inferred from the codebase:
